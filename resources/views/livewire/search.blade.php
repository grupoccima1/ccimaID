<div class="mt-4">
    <form class="pt-2 relative mx-5 text-gray-600" autocomplete="off"> 
        ...
        <button type="submit" class=" bg-blue-500 hover:to-blue-700 text-white font-bold py-2 px-4 rounded absolute right-0 top-0 mt-2 ">
            Buscar
        </button> --}}
        <!-- component -->
            ...
            <ul class="absolute left-0 w-full bg-white mt-1 rounded-lg overflow-hidden">
                @forelse ($this->results as $result)
                    <a href="{{asset($result->url)}}">
                        <li class="leading-10 px-5 text-sm cursor-pointer hover:bg-gray-300"> {{$result->name}}</li>
                    </a>    
                @empty
